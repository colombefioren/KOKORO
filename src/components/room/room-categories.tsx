...
import { Home, Mail, PlayCircle, Star } from "lucide-react";
...
const RoomCategories = ({
  ...
  const categories = [
    ...
    { id: "active", label: "Active Now", icon: PlayCircle, count: 4 },
          ...
          {categories.map((category) => (
            ...
            >
              <div className="flex items-center gap-2 w-full justify-center">
                <category.icon className={`w-4 h-4 transition-transform duration-300 ${
                  activeCategory === category.id 
                ...
                <span className={`px-2 py-1 rounded-full text-xs font-bold min-w-[24px] transition-all duration-300 ${
                  activeCategory === category.id
